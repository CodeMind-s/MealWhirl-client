...
export default function CartPage() {
      ...
      try {
        ...
        const cartItems = data.itemsByRestaurant.flatMap((restaurant) =>
          restaurant.items.map((item: any) => ({
            ...
            restaurantName:
              restaurant.restaurantName || restaurant.restaurantId, // Fallback if no restaurantName
  ...
  const updateQuantity = async (id: string, quantity: number) => {
    try {
      ...
      ).itemsByRestaurant.flatMap((restaurant: any) =>
        restaurant.items.map((item: any) => ({
          id: item.menuItemId,
          restaurantId: restaurant.restaurantId,
        ...
        <div className="lg:col-span-2 space-y-6">
          {Object.entries(itemsByRestaurant).map(
            ([restaurantId, { restaurantName, items }]) => (
              <Card
                key={restaurantId}
                className="overflow-hidden border-blue-100 dark:border-blue-900"
              >
